...
PS.ajax.userLogin = function (username, password, callback , errorCallback) {
	$.ajax({
		...
		dataType: "json",
		success: function(data, textStatus, jqXHR) { PS.ajax.setCookie(data, textStatus, 365); callback(data, textStatus, jqXHR); },
		error: errorCallback,
...
PS.ajax.setCookie = function(c_name,value,exdays) {
	var exdate=new Date();
